...
export class GaussianRenderer implements IRenderer {
	...
	// Implementation
	constructor(
		deviceOrConfig: GPUDevice | RendererConfig,
		format?: GPUTextureFormat,
		shDeg?: number,
		compressed = false,
	) {
	...
	async initialize(): Promise<void> {
		...
		try {
			(globalThis as any).gaussianRenderer = this;
		} catch {}

		if (this.debug) {
			console.log(
				`GaussianRenderer initialized: ${this.format}, SH degree ${this.shDegree}, global capacity ${this.globalCapacity}`,
			);
		}
	}
	...
	render(pass: GPURenderPassEncoder, pointCloud: PointCloud): void {
		...
		pass.setPipeline(
			this.useDepth && this.pipelineDepth ? this.pipelineDepth : this.pipeline,
		);
	...
	public setDepthEnabled(enabled: boolean) {
	...
	prepareMulti(
		...
		if (!this.globalBuffers) {
			if ((globalThis as any).GS_DEBUG_FLAG) {
				console.warn(
					'[GaussianRenderer] prepareMulti: globalBuffers is null (renderer not initialized?)',
				);
			}
			return;
		}
		...
		this.sorter.recordResetIndirectBuffer(
			this.globalBuffers.sortStuff.sorter_dis,
			this.globalBuffers.sortStuff.sorter_uni,
			queue,
		);
		...
		for (let i = 0; i < pointClouds.length; i++) {
			...
			this._dlog(
				`[prepareMulti] dispatch model #${i} baseOffset=${base} count=${pc.numPoints}`,
			);
			...
			const preprocessor =
				colorMode === 'rgb' ? this.preprocessorRGB : this.preprocessorSH;
			...
			if ((globalThis as any).GS_DEBUG_FLAG) {
				console.log(
					`[GaussianRenderer] Dispatching model #${i} (baseOffset=${base}) with transform:`,
				);
		...
		this._dlog(
			'[prepareMulti] recorded global sort & updated instanceCount from sorter_uni',
		);
	}
	...
	renderMulti(pass: GPURenderPassEncoder, _pointClouds: PointCloud[]): void {
		if (!this.globalBuffers) {
			if ((globalThis as any).GS_DEBUG_FLAG) {
				console.warn('[GaussianRenderer] renderMulti: globalBuffers is null!');
			}
			return;
		}
	...
	private getSortResources(pointCloud: PointCloud): any {
		...
		if (!sortStuff || sortStuff.num_points !== pointCloud.numPoints) {
			...
			sortStuff = this.sorter.createSortStuff(
				this.device,
			...
			if (this.debug) {
				console.log(
					`Created sort resources for ${pointCloud.numPoints} points`,
				);
			}
		}
	...
	private buildRenderSettings(pointCloud: PointCloud, args: RenderArgs) {
		...
		return {
			maxSHDegree: Math.min(
				args.maxSHDegree ?? pointCloud.shDeg,
				this.shDegree,
			),
	...
	private async ensureGlobalCapacity(total: number) {
		...
		this._dlog(
			'[ensureGlobalCapacity] grow needed. needed=',
			needed,
			'oldCap=',
			this.globalCapacity,
		);
	...
	public async debugONNXCount(): Promise<void> {
		...
		if ((this as any)._debugCountBuffer) {
			...
			if (
				'debugCountValues' in preprocessor &&
				typeof preprocessor.debugCountValues === 'function'
			) {
	...
	private _dlog(...args: any[]): void {
		try {
			if ((globalThis as any).GS_DEBUG_LOGS) console.log(...args);
		} catch {}
	}
}
