...
export class GaussianThreeJSRenderer extends THREE.Mesh {
	...
	public renderThreeScene(camera: THREE.Camera): void {
		...
		if (
			!this.sceneDepthRT ||
			this.sceneDepthRT.width !== width ||
			this.sceneDepthRT.height !== height
		) {
			...
			this.sceneDepthTexture = new THREE.DepthTexture(
				width,
				height,
				THREE.FloatType,
			);
	...
	private blitRenderTargetToCanvas(camera: THREE.Camera): void {
		...
		try {
			...
			this.blitWithRenderPass(
				device,
				rtColorTexture,
				canvasView,
				this.sceneDepthRT.width,
				this.sceneDepthRT.height,
			);
			...
			console.warn(
				'[Depth] Blit with render pass failed, falling back to re-render:',
				error,
			);
	...
	private blitWithRenderPass(
		...
		const encoder = device.createCommandEncoder({
			label: 'RT-to-Canvas render pass',
		});
	...
	public onBeforeRender(
		...
		if (!this.pcs || this.pcs.length === 0 || totalPoints === 0) {
			if ((globalThis as any).GS_VIDEO_EXPORT_DEBUG) {
				...
				console.log(
					'[GaussianThreeJSRenderer] - gaussianModels数量:',
					this.gaussianModels.length,
				);
				console.log(
				...
				visibleModels.forEach((model, index) => {
					...
					console.log(
						`[GaussianThreeJSRenderer] - 模型${index} getPointCloud返回类型:`,
						pc.constructor.name,
						'is PointCloud:',
						pc instanceof PointCloud,
						'is DynamicPointCloud:',
						pc instanceof DynamicPointCloud,
						'is FBXModelWrapper:',
						pc instanceof FBXModelWrapper,
					);
				});
			}
			return;
	...
	public drawSplats(
		...
		if (
			...
			console.warn(
				'drawSplats: Only THREE.PerspectiveCamera is supported!',
				camera,
			);
			return false;
		}
		...
		if (this.sceneDepthTexture) {
			...
			try {
				...
				if (depthGpuTexture && threeDepthFormat) {
					...
					if ((globalThis as any).GS_DEPTH_DEBUG) {
						console.warn(
							'[Depth] ⚠️ Could not access depth GPU texture from Three.js backend',
						);
					}
				}
	...
	private ensureGizmoOverlayRenderTarget(width: number, height: number): void {
		...
		if (
			this.gizmoOverlayRT &&
			this.gizmoOverlayRT.width === w &&
			this.gizmoOverlayRT.height === h
		) {
			return;
		}
	...
	private compositeOverlayToCanvas(
		device: GPUDevice,
	...
	public setOccluderMeshes(meshes: THREE.Mesh[]): void {
		console.warn(
			'[GaussianThreeJSRenderer] setOccluderMeshes is deprecated. Auto depth mode captures the full scene automatically.',
		);
		console.warn(
			'[GaussianThreeJSRenderer] To use manual occluders, set autoDepthMode = false',
		);
	...
	public diagnoseDepth(): void {
		...
		if (this.renderer) {
			console.log(
				'GaussianRenderer depth enabled:',
				(this.renderer as any).useDepth,
			);
			console.log(
				'GaussianRenderer depth format:',
				(this.renderer as any).depthFormat,
			);
		}
	...
	private getViewport(): [number, number] {
		...
		const width =
			dbSize.x ||
			(this.threeRenderer.domElement?.width ?? 0) ||
			this.threeRenderer.getSize(new THREE.Vector2()).x;
		const height =
			dbSize.y ||
			(this.threeRenderer.domElement?.height ?? 0) ||
			this.threeRenderer.getSize(new THREE.Vector2()).y;
		return [width, height];
	}

	private convertCamera(
		camera: THREE.Camera,
		renderer: THREE.WebGPURenderer,
	): PerspectiveCamera {
	 ...
	 */
	public async updateDynamicModels(
		camera: THREE.Camera,
		time?: number,
	): Promise<void> {
	...
	public setModelGaussianScale(modelId: string, scale: number): void {
		...
		if (modelIndex >= 0 && modelIndex < this.gaussianModels.length) {
			...
			console.log(
				`[GaussianThreeJSRenderer] Model ${modelId} Gaussian scale set to: ${scale}`,
			);
		} else {
	...
	public setModelMaxShDeg(modelId: string, deg: number): void {
		...
		if (modelIndex >= 0 && modelIndex < this.gaussianModels.length) {
			...
			console.log(
				`[GaussianThreeJSRenderer] Model ${modelId} Max SH degree set to: ${deg}`,
			);
		} else {
			console.warn(`[GaussianThreeJSRenderer] Model ${modelId} not found`);
		}
	...
	public setModelKernelSize(modelId: string, size: number): void {
		...
		if (modelIndex >= 0 && modelIndex < this.gaussianModels.length) {
			...
			console.log(
				`[GaussianThreeJSRenderer] Model ${modelId} Kernel size set to: ${size}`,
			);
		} else {
			console.warn(`[GaussianThreeJSRenderer] Model ${modelId} not found`);
		}
	...
	public setModelOpacityScale(modelId: string, scale: number): void {
		...
		if (modelIndex >= 0 && modelIndex < this.gaussianModels.length) {
			...
			console.log(
				`[GaussianThreeJSRenderer] Model ${modelId} Opacity scale set to: ${scale}`,
			);
		} else {
			console.warn(`[GaussianThreeJSRenderer] Model ${modelId} not found`);
		}
	...
	public setModelCutoffScale(modelId: string, scale: number): void {
		...
		if (modelIndex >= 0 && modelIndex < this.gaussianModels.length) {
			...
			console.log(
				`[GaussianThreeJSRenderer] Model ${modelId} Cutoff scale set to: ${scale}`,
			);
		} else {
			console.warn(`[GaussianThreeJSRenderer] Model ${modelId} not found`);
		}
	...
	public setModelTimeScale(modelId: string, scale: number): void {
		...
		if (modelIndex >= 0 && modelIndex < this.gaussianModels.length) {
			...
			console.log(
				`[GaussianThreeJSRenderer] Model ${modelId} Time scale set to: ${scale}`,
			);
		} else {
			console.warn(`[GaussianThreeJSRenderer] Model ${modelId} not found`);
		}
	...
	public setModelTimeOffset(modelId: string, offset: number): void {
		...
		if (modelIndex >= 0 && modelIndex < this.gaussianModels.length) {
			...
			console.log(
				`[GaussianThreeJSRenderer] Model ${modelId} Time offset set to: ${offset}`,
			);
		} else {
			console.warn(`[GaussianThreeJSRenderer] Model ${modelId} not found`);
		}
	...
	public setModelAnimationIsLoop(modelId: string, is_loop: boolean): void {
		...
		if (modelIndex >= 0 && modelIndex < this.gaussianModels.length) {
			...
			console.log(
				`[GaussianThreeJSRenderer] Model ${modelId} Animation is loop set to: ${is_loop}`,
			);
		} else {
			console.warn(`[GaussianThreeJSRenderer] Model ${modelId} not found`);
		}
	...
	public setModelTimeUpdateMode(modelId: string, mode: any): void {
		...
		if (modelIndex >= 0 && modelIndex < this.gaussianModels.length) {
			...
			console.log(
				`[GaussianThreeJSRenderer] Model ${modelId} Time update mode set to: ${mode}`,
			);
		} else {
			console.warn(`[GaussianThreeJSRenderer] Model ${modelId} not found`);
		}
	...
	public setModelRenderMode(modelId: string, mode: number): void {
		...
		if (modelIndex >= 0 && modelIndex < this.gaussianModels.length) {
			...
			console.log(
				`[GaussianThreeJSRenderer] Model ${modelId} Render mode set to: ${mode}`,
			);
		} else {
			console.warn(`[GaussianThreeJSRenderer] Model ${modelId} not found`);
		}
	...
	public startModelAnimation(modelId: string, speed: number = 1.0): void {
		...
		if (modelIndex >= 0 && modelIndex < this.gaussianModels.length) {
			...
			console.log(
				`[GaussianThreeJSRenderer] Model ${modelId} Animation started at ${speed}x speed`,
			);
		} else {
			console.warn(`[GaussianThreeJSRenderer] Model ${modelId} not found`);
		}
	...
	public pauseModelAnimation(modelId: string): void {
		...
		if (modelIndex >= 0 && modelIndex < this.gaussianModels.length) {
			...
			console.log(
				`[GaussianThreeJSRenderer] Model ${modelId} Animation paused`,
			);
		} else {
			console.warn(`[GaussianThreeJSRenderer] Model ${modelId} not found`);
		}
	...
	public resumeModelAnimation(modelId: string): void {
		...
		if (modelIndex >= 0 && modelIndex < this.gaussianModels.length) {
			...
			console.log(
				`[GaussianThreeJSRenderer] Model ${modelId} Animation resumed`,
			);
		} else {
			console.warn(`[GaussianThreeJSRenderer] Model ${modelId} not found`);
		}
	...
	public stopModelAnimation(modelId: string): void {
		...
		if (modelIndex >= 0 && modelIndex < this.gaussianModels.length) {
			...
			console.log(
				`[GaussianThreeJSRenderer] Model ${modelId} Animation stopped`,
			);
		} else {
			console.warn(`[GaussianThreeJSRenderer] Model ${modelId} not found`);
		}
	...
	public setModelAnimationTime(modelId: string, time: number): void {
		...
		if (modelIndex >= 0 && modelIndex < this.gaussianModels.length) {
			...
			console.log(
				`[GaussianThreeJSRenderer] Model ${modelId} Animation time set to: ${time.toFixed(3)}s`,
			);
		} else {
			console.warn(`[GaussianThreeJSRenderer] Model ${modelId} not found`);
		}
	...
	public setModelAnimationSpeed(modelId: string, speed: number): void {
		...
		if (modelIndex >= 0 && modelIndex < this.gaussianModels.length) {
			...
			console.log(
				`[GaussianThreeJSRenderer] Model ${modelId} Animation speed set to: ${speed}x`,
			);
		} else {
			console.warn(`[GaussianThreeJSRenderer] Model ${modelId} not found`);
		}
	...
	public removeModelById(modelId: string): boolean {
		...
		if (
			isNaN(modelIndex) ||
			modelIndex < 0 ||
			modelIndex >= this.gaussianModels.length
		) {
			console.warn(
				`[GaussianThreeJSRenderer] removeModelById: invalid id ${modelId}`,
			);
			return false;
		}
		...
		try {
			...
			console.warn(
				'[GaussianThreeJSRenderer] Error removing model from scene:',
				e,
			);
		}
	...
	public setModelVisible(modelId: string, visible: boolean): void {
		...
		if (modelIndex >= 0 && modelIndex < this.gaussianModels.length) {
			...
			console.log(
				`[GaussianThreeJSRenderer] Model ${modelId} visible: ${visible}`,
			);
		} else {
			console.warn(`[GaussianThreeJSRenderer] Model ${modelId} not found`);
		}
	...
	public startAllAnimations(speed: number = 1.0): void {
		...
		console.log(
			`[GaussianThreeJSRenderer] All animations started at ${speed}x`,
		);
	}
	...
	public setAllAnimationTime(time: number): void {
		...
		console.log(
			`[GaussianThreeJSRenderer] Global animation time set: ${time.toFixed(3)}s`,
		);
	}
	...
	public setAllAnimationSpeed(speed: number): void {
		...
		console.log(
			`[GaussianThreeJSRenderer] Global animation speed set: ${speed}x`,
		);
	}
}
