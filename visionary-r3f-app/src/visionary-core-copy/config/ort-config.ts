...
export function initOrtEnvironment(wasmPaths?: string | string[]): void {
	...
	if (typeof window !== 'undefined' && (window as any).ort) {
		...
		console.log(
			`[VisionaryCore] ONNX Runtime WASM paths configured: ${getOrtWasmPaths()}`,
		);
	} else {
		console.warn(
			'[VisionaryCore] ONNX Runtime not available, configuration will be applied when ort is loaded',
		);
		...
		if (typeof window !== 'undefined') {
			const applyConfigWhenReady = () => {
				if ((window as any).ort) {
					...
					ort.env.wasm.wasmPaths = getOrtWasmPaths();
					console.log(
						`[VisionaryCore] ONNX Runtime WASM paths configured (delayed): ${getOrtWasmPaths()}`,
					);
				} else {
					// 如果 ort 还没有加载，继续等待
