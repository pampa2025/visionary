...
export function AsyncWebGPUCanvas({
	...
	const [status, setStatus] = useState<
		'checking' | 'initializing' | 'ready' | 'error'
	>('checking');
	...
	useEffect(() => {
		const checkSupport = async () => {
			if (!navigator.gpu) {
				setStatus('error');
				setErrorMessage(
					'WebGPU is not supported in this browser. Try Chrome 113+ or Edge.',
				);
				return;
			}
			...
			try {
				...
				onInitError?.(
					error instanceof Error ? error : new Error('WebGPU check failed'),
				);
			}
		};
	...
	const createRenderer = useCallback(
		(params: any) => {
			...
			const renderer = new WebGPURenderer({
				...
				powerPreference: 'high-performance',
				// @ts-ignore
				requiredFeatures: ['bgra8unorm-storage', 'float32-filterable'],
			});

			// Initialize via VisionaryInitializer
			VisionaryInitializer.getInstance()
				.initializeWithExistingRenderer(renderer, {
					debugMode: true,
				})
				.then(() => {
				...
				{fallback || (
					<div style={{ marginTop: '20px' }}>
						...
						<ul style={{ textAlign: 'left', display: 'inline-block' }}>
							...
							<li>
								Enable <code>#enable-unsafe-webgpu</code> in{' '}
